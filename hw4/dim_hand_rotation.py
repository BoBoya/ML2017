...
for i in range(60):
    ...
    for N in [10000, 20000, 50000, 80000, 100000]:
        layer_dims = [np.random.randint(60, 80), 32*32]
...
for i in range(1, 482):
    v1 = im.open('hand/hand.seq{}.png'.format(i))
    v1 = v1.resize( (32, 32), im.BILINEAR )
    v1.save('shand/shand.seq{}.png'.format(i))

collect = []
for i in range(1, 482):
    v2 = im.open('shand/shand.seq{}.png'.format(i))
    tmp = np.array(v2)
    tmp = tmp.flatten()
    collect.append(tmp)
